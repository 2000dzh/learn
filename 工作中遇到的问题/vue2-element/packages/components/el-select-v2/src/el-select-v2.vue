  ...
  methods: {
      ...
      const selectedOptions = this.flattedOptions
        .filter(
          (option) =>
            ...
            values.some((value) => this.isSameValue(value, option[this.valueKey]))
        )
        .map((option) => ({
    ...
    handleScrollerVisible() {
      const firstValue = this.multiple
        ? Array.isArray(this.localValue) && this.localValue[0]
        : this.localValue
      this.localIndex = this.localOptions.findIndex((option) =>
        this.isSameValue(option[this.valueKey], firstValue)
      )
    ...
    localFilterMethod(query) {
      ...
      if (typeof this.filterMethod === 'function') {
        ...
        const groupNameList = this.flattedOptions.reduce((sum, option) => {
          if (
            !option._isGroup &&
            option[this.labelKey] &&
            option[this.labelKey].toLowerCase().includes(query.toLowerCase())
          ) {
            sum.push(option._groupName)
          }
  ...
  computed: {
    flattedOptions() {
      ...
      this.options.forEach((option) => {
        const _groupName = buildUUID()
        if (Array.isArray(option[this.optionsKey])) {
          list.push({
            ...option,
            _isGroup: true,
            _groupName,
            [this.valueKey]: buildUUID(),
          })
          list.push(
            ...option[this.optionsKey].map((subOption) => ({
              ...subOption,
              _groupName,
            }))
          )
      ...
      if (
        this.allowCreate &&
        this.query &&
        !list.some((option) => option[this.labelKey] === this.query)
      ) {
        list.unshift({
